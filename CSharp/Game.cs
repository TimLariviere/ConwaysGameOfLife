...
namespace CSharp
{
    public class Game
    {
        ...
        public static readonly HashSet<int> RulesLiveCell = new HashSet<int>(new[] { 2, 3, 5 });
        public static readonly HashSet<int> RulesDeadCell = new HashSet<int>(new[] { 0, 3 });
        ...
        public void PlayStep()
        {
            ...
            for (int x = 0; x < MapWidth; x++)
            {
                for (int y = 0; y < MapHeight; y++)
                {
                    ...
                    if (currentCell && !RulesLiveCell.Contains(neighborsCount))
                    {
                        Map[x, y] = false;
                    }
                    else if (!currentCell && RulesDeadCell.Contains(neighborsCount))
                    {
                        Map[x, y] = true;
                    }
                }
